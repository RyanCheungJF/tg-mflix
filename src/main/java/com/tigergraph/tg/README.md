...
To learn how you can load data into this instance of TigerGraph, refer to this [page](data\tigergraph\gsql\README.md).
...
tigergraph.datasource.graphName=internship
...
`GET /movies/q={...}`: Searches for a movie that fits into the category based on the pattern given.
This endpoint uses the `LIKE` keyword to do a search, and thus docs can be referred to [here](https://docs.tigergraph.com/gsql-ref/current/querying/operators-and-expressions#_like).
